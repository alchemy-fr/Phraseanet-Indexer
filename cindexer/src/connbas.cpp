	...
	if( (this->mysqlCnx = mysql_init((MYSQL *)0)) )
	{
		...
		if( mysql_real_connect( this->mysqlCnx, host, user, passwd, szDB, port, NULL, CLIENT_COMPRESS ) )
		{
			if(arg_mycharset != NULL)
			{
				...
				{
					zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "Can't set character set %s on database %s !", arg_mycharset, szDB ) ;
					ret = -4 ;
				}
			}
			if(ret==0)
			{
				...
				{
					zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "Can't select the database %s !", szDB ) ;
					ret = -2 ;
				}
			}
		}
		...
		{
			zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "Can't connect to the mysql server on port %d !",	MYSQL_PORT ) ;
			ret = -1 ;
		}
	}
	else
	{
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "Can't init the mysql server" ) ;
		ret = -3 ;
	}
	if(ret == 0)
		this->isok = true;
...
int CConnbas::execute(char *sql, int lenght)
{
	...
	if(this->crashed)
	{
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CConnbas::execute : rejected (base is on error state)");
		return(-1);
	}
	try
	{
		...
		{
			zSyslog._log(CSyslog::LOGL_SQLOK, CSyslog::LOGC_SQLOK, "CConnbas::execute : %s : %f sec", sql, stopChrono(chrono));
		}
		return(ret);
	}
	catch(...)
	{
		this->crashed = 1;
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "exception in CConnbas::execute");
		return(-1);
	}
}
...
void CConnbas::close()
{
	...
	if(this->mysqlCnx)
	{
		if(this->crashed)
		{
			zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CConnbas::close : rejected (base is on error state)");
			return;
		}
		try
		{
		...
		{
			this->crashed = 1;
			zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "exception in CConnbas::close");
		}
		this->mysqlCnx = NULL;
	}
}
...
int CMysqlStmt::execute()
{
	...
	{
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::execute : rejected (base is on error state)");
		return(-1);
	}
	try
	{
		...
		{
			zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::execute : mysql_stmt_execute error (%d) : '%s', return(%d)", this->errNo(), this->error(), r);
		...
		{
			zSyslog._log(CSyslog::LOGL_SQLOK, CSyslog::LOGC_SQLOK, "CMysqlStmt::execute : %s : %f sec", this->sql, stopChrono(chrono));
		}
		return(r);
	}
	catch(...)
	{
		this->connbas->crashed = 1;
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "exception in CMysqlStmt::execute");
		return(-1);
	}
}
...
int CMysqlStmt::bind_param()
{
	...
	if(this->connbas->crashed)
	{
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::bind_param : rejected (base is on error state)");
		return(-1);
	}
	try
	{
		...
		{
			zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::bind_param : mysql_stmt_bind_param error (%d) : '%s'", this->errNo(), this->error());
			switch(this->errNo())
			{
				// erreurs fatales reconnues
				case ER_SERVER_SHUTDOWN:		// 1053 = 'Server shutdown in progress'
	...
	{
		this->connbas->crashed = 1;
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "exception in CMysqlStmt::bind_param");
		return(-1);
	}
}
...
int CMysqlStmt::bind_result()
{
	...
	if(this->connbas->crashed)
	{
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::bind_result : rejected (base is on error state)");
		return(-1);
	}
	try
	{
		...
		{
			zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::bind_result : mysql_stmt_bind_result error (%d) : '%s'", this->errNo(), this->error());
	...
	{
		this->connbas->crashed = 1;
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "exception in CMysqlStmt::bind_result");
		return(-1);
	}
}
...
int CMysqlStmt::store_result()
{
	...
	if(this->connbas->crashed)
	{
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::store_result : rejected (base is on error state)");
		return(-1);
	}
	try
	{
		...
		{
			zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::store_result : mysql_stmt_store_result (%d) : '%s'", this->errNo(), this->error());
			switch(this->errNo())
			{
				// erreurs fatales reconnues
				case ER_SERVER_SHUTDOWN:		// 1053 = 'Server shutdown in progress'
	...
	catch(...)
	{
		...
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "exception in CMysqlStmt::store_result");
		return(-1);
	}
}
...
int CMysqlStmt::fetch()
{
	...
	if(this->connbas->crashed)
	{
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::fetch : rejected (base is on error state)");
		return(-1);
	}
	try
	{
	...
	{
		this->connbas->crashed = 1;
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "exception in CMysqlStmt::fetch");
		return(-1);
	}
}
...
void CMysqlStmt::free_result()
{
	...
	if(this->connbas->crashed)
	{
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::free_result : rejected (base is on error state)");
		return;
	}
	try
	{
	...
	{
		this->connbas->crashed = 1;
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "exception in CMysqlStmt::free_result");
	}
}
...
void CMysqlStmt::data_seek(my_ulonglong offset)
{
	...
	if(this->connbas->crashed)
	{
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "CMysqlStmt::data_seek : rejected (base is on error state)");
		return;
	}
	try
	{
	...
	{
		this->connbas->crashed = 1;
		zSyslog._log(CSyslog::LOGL_ERR, CSyslog::LOGC_SQLERR, "exception in CMysqlStmt::data_seek");
	}
}
